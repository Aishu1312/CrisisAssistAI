...
class MainAgentController:
    """
    The orchestrator and central coordinator for CrisisAssist AI.
    Runs the multi-agent execution pipeline, registers tool traces, 
    and handles A2A messages.
    """
    def __init__(self):
        self.api_key = os.getenv("GEMINI_API_KEY")
        self.client = None
        
        # Instantiate Gemini GenAI client if API key is provided
        if self.api_key:
            try:
                self.client = genai.Client(api_key=self.api_key)
            except Exception as e:
                print(f"Failed to initialize Gemini Client: {e}. Running in offline/fallback mode.")
                
        # Initialize memory systems
        self.session_memory = SessionMemory()
        self.user_memory = UserMemory()
        
        # Initialize core utilities and tools
        self.translation_tool = TranslationTool(self.client)
        self.voice_tool = VoiceTool()
        self.observability = Observability()
        
        # Initialize worker and planner agents
    ...
    def process_emergency_request(self, user_query: str) -> Dict[str, Any]:
        """
        Coordinates the multi-agent pipeline:
        Triage -> Planning -> Execution -> Evaluation -> Translation -> TTS.
        """
        ...
        # Stage 0: Language Detection and Input Normalization
        self.session_memory.add_step("MainController", "Detecting language", "STARTED")
        ...
        # Normalize input to English for agent consistency
        ...
        if detected_lang != "en":
            ...
            self.session_memory.add_step("MainController", "Language check: English", "COMPLETED")
            
        # Stage 1: Priority Detection (Triage)
        ...
        # Save emergency metadata to session
        ...
        self.session_memory.add_step("WorkerAgent", "Tool execution & draft compilation", "STARTED")
        ...
        # Stage 4: Evaluation & Refinement Loop
        eval_start = time.time()
        self.session_memory.add_step("EvaluatorAgent", "Quality & safety review", "STARTED")
        ...
        # Loop check: If not approved, execute refinement step
        if not eval_approved:
            self.session_memory.add_step(
                "EvaluatorAgent", 
                "Review FAILED", 
                "REJECTED", 
                {"score": eval_score, "feedback": eval_feedback}
            )
            # Route back to Worker Agent with evaluator feedback
            ...
            eval_msg_out = self.evaluator_agent.run(eval_msg_in)
            eval_score = eval_msg_out.payload.get("score", 0.0)
            eval_approved = eval_msg_out.payload.get("approved", True)  # Enforce approval on second pass
        ...
        stages_log.append({"stage": "Safety & Quality Review", "duration_ms": eval_duration})

        # Save resources in session
        self.session_memory.set_verified_resources(verified_resources)

        # Stage 5: Output Translation & Audio Preparation
        ...
        if detected_lang != "en":
            self.session_memory.add_step("MainController", f"Translating output to {detected_lang}", "STARTED")
            translated_output = self.translation_tool.translate(final_text, "en", detected_lang)
        ...
        tts_text = f"Priority {priority_tier} emergency resolved. Here are your action steps: " + worker_payload.get("summary_checklist", "")
        audio_file = self.voice_tool.text_to_speech(tts_text, detected_lang)
        if audio_file:
        ...
        # Store past request context in long-term memory
        ...
        # Finalize response
        self.session_memory.add_agent_message("CrisisAssistAgent", translated_output, detected_lang, audio_file)
        
        total_duration = int((time.time() - start_time) * 1000)
        
        # Register in Observability log database
